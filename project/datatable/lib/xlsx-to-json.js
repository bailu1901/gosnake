...
function toJson(excel, head, dest) {
    for (var i_sheet = 0; i_sheet < excel.worksheets.length; i_sheet++) {
        ...
        if (sheet.data && sheet.data.length > 0) {
                ...
                for (var i_col = 0; i_col < sheet.maxCol; i_col++) {
                    ...
                    switch (type) {
                        case 'basic': // number string boolean
                            row_obj[i_col] = cell.value;
                            break;
                        case '{}':
                            if (cell) {
                                addObjectField(row_obj, i_col, cell.value);
                            };
                            break;
                        case '[]': //[number] [boolean] [string]
                            if (cell) {
                                addBasicArrayField(row_obj, i_col, cell.value);
                            };
                            break;
                        case '[{}]': //[object]
                            if (cell) {
                                addObjectArrayField(row_obj, i_col, cell.value);
                            };
                            break;
                    };
                };
            ...
            sb.push(sheet.name);
            sb.push("_data = ");
            sb.push(JSON.stringify(output));
            sb.push(";");

            sb.push("var ");
            sb.push(sheet.name);
            sb.push("_desc = ");
            sb.push(JSON.stringify(col_name));
            sb.push(";");
            ...
            sb.push('var {1} = {}; for (var i = {0}.length - 1; i >= 0; i--) {var row = {};for (var j = {0}[i].length - 1; j >= 0; j--) {row[{2}[j]] = {0}[i][j];};{1}[{0}[i][0]] = row;};'.format(sheet.name + "_data", sheet.name, sheet.name + "_desc"));
            ...
            var dest_file = path.resolve(dest, sheet.name + ".js");
